...
export default function MovePlantsPage() {
  ...
  const handleScan = (tag) => {
    ...
    // AUTO CAPTURE AFTER SMALL DELAY
    setTimeout(() => {
      const image = captureImage()

      setItems(prev => [
        ...prev,
        { tag, image, time: new Date().toISOString() }
      ])
  ...
  // 🔵 MOVE TO METRC
  const move = async () => {
    if (!items.length) return setMsg('No items')
    if (!location) return setMsg('Select location')

    setLoading(true)
    setMsg('Sending to METRC...')
    ...
    try {
      ...
      setItems([])
      setMsg(`Moved ${labels.length} plants`)
    } catch (e) {
      setMsg(e.message)
    } finally {
      setLoading(false)
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4 p-4">
      ...
      <div className="card space-y-3">
        ...
        <input
          placeholder="Destination (EXACT METRC NAME)"
